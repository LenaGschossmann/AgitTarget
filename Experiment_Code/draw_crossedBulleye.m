...
function draw_crossedBulleye(screenRes, window, windowRect, targetVisAngle, px_per_deg, duration)
...
ctr_XY = (diamL+1)/2;
line_thickness = round(radS/2);
circleRect(sqrt((X-ctr_XY).^2 + (Y-ctr_XY).^2) <= radL) = black;
circleRect(ctr_XY-line_thickness:ctr_XY+line_thickness, :) = white;
circleRect(:,ctr_XY-line_thickness:ctr_XY+line_thickness) = white;
circleRect(sqrt((X-ctr_XY).^2 + (Y-ctr_XY).^2) <= radS) = black;
