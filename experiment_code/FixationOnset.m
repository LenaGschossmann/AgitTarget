...
draw_instructions_and_wait(['Welcome to the lab. \n\n', ...
    'In the following, each stimulus presented will show a natural scenes. \n\n'...
    'You are allowed to freely explore the scence. \n Before each scene, you will see a fixation dot. Please remain fixated until it disappears. \n\n' ...
    ...
    'This phase will take about 30 minutes to complete. \n\n' ...
...
if eyetracking==1
    setup_eyetracker;
    %open log file
    Eyelink('OpenFile', sprintf('WL_FO_s%u.EDF',subject_id));          %CHANGE file name ?
...
for block = 1:blockNumber
    ...
    for trial_i = 1:block_count(1)  %length(stimID)*repStim
        ...
        Screen('DrawTexture',win, WLindex(stim_index), [0,0,size(currentIMG,2),size(currentIMG,1)],[0,0,ScreenSize(3),ScreenSize(4)]);
...
outputname = [resultpath sprintf('/ITW_WLFO_subj%u',subject_id)];
%
%
...
if eyetracking==1 && calibrate_eyelink
    ...
    Eyelink('ReceiveFile',sprintf('WL_FO_s%u.EDF',subject_id),fulledffile);
    Eyelink('WaitForModeReady', 500);
end
